...
Five real CVs from the training dataset, one per role, scored with the live deployed model (`backend/models/cv_job_score_model_v2.pkl` — the same one `/analyze-cv` serves) and Phase 4's percentile lookup. Full raw output: [`demo_results.json`](demo_results.json). Regenerate this report with [`backend/build_demo.py`](../../backend/build_demo.py) after changing the deployed model.
