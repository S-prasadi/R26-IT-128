...
export const upsertGoalSchema = z.object({
  ...
  target_industry:  z.string().trim().max(120).optional(),
...
export const predictPathSchema = z.object({
  ...
  add_skills:        z.array(z.string().trim().min(1).max(80)).max(20).optional(),
  remove_skills:     z.array(z.string().trim().min(1).max(80)).max(50).optional(),
  simulate:          z.boolean().optional(),
});

export const generateRoadmapSchema = z.object({
  path_id:       z.string().trim().min(1),
  // Which prediction snapshot path_id should resolve against — the id /predict
  // returns for the run it came from. Optional for backward compatibility
  // (falls back to the latest snapshot), but should always be sent by the
  // frontend so this doesn't silently resolve against a newer prediction.
