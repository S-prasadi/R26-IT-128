...
function blendReadiness(
  ...
  const coverage = total ? skillsMatched.length / total : 0;
...
function normalizeModuleBResponse(raw: ModuleBResponse, currentRole: string) {
  ...
  const totalConf = raw.paths.reduce((sum, p) => sum + (p.confidence || 0), 0);
  ...
  const paths = raw.paths.map((p) => {
    ...
    return {
      ...
      probability:         p.confidence,
      confidence_relative: totalConf > 0 ? Number((p.confidence / totalConf).toFixed(4)) : 0,
      transitions,
...
export const careerService = {
  ...
  async predictPath(userId: string, dto: PredictPathDto, userSkills: object[]) {
    ...
    const base = [...userSkillNames, ...cvSkillNames].map(norm).filter(Boolean).filter((s) => !removeSet.has(s));
    ...
    const { nodeMetaByRole, ...result } = normalizeModuleBResponse(raw, currentRole);
